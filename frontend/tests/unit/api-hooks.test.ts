...
describe('API Hooks - T141', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('Mastery Calculation Hook - useMasteryCalculation', () => {
    it('should fetch mastery calculation data successfully', async () => {
    ...
    it('should not execute query if studentId is missing', () => {
      const { result } = renderHook(() =>
        useMasteryCalculation({
    ...
    it('should handle invalid parameters', async () => {
      ...
      const { result } = renderHook(() =>
        useMasteryCalculation({
          ...
          difficulty: 'invalid-difficulty' as any, // Invalid difficulty
        }),
        { wrapper }
      );
  ...
  describe('Batch Processing Hook - useBatchProcessing', () => {
    ...
    it('should track batch progress', async () => {
      const mockProgress = {
        ...
        errors: [],
        estimatedCompletion: '2026-01-15T10:30:00Z',
      };
      ...
      await act(async () => {
        result.current.trackProgress('batch-123');
      });

      await waitFor(() => expect(result.current.isSuccess).toBe(true));

      expect(api.get).toHaveBeenCalledWith('/api/batch/progress?batchId=batch-123');
  ...
  describe('Predictive Analytics Hook - usePredictiveAnalytics', () => {
    ...
    it('should handle empty topics array', async () => {
      const mockResponse = {
        ...
        timeframe: '7d' as const,
        predictions: [],
        overallTrajectory: { direction: 'stable' as const, velocity: 0, acceleration: 0, sustainability: 0 },
        riskFactors: [],
        recommendations: [],
        confidence: { overall: 0, byTopic: {}, sampleSize: 0, reliability: 'low' as const },
        generatedAt: '2026-01-15T10:00:00Z',
      };
      ...
      expect(result.current.data?.predictions).toEqual([]);
    });
  });
  ...
  describe('Recommendations Hook - useRecommendations', () => {
    ...
    it('should handle empty context', async () => {
      const mockResponse = {
        ...
        generatedAt: '2026-01-15T10:00:00Z',
        recommendations: [],
        summary: { totalRecommendations: 0, byType: {}, byPriority: {}, byTopic: {}, difficultyDistribution: {}, estimatedTotalTime: 0 },
        metadata: { modelVersion: '1.0', confidence: 0.5, processingTime: 0.1, context: {} },
      };
  ...
  describe('Performance Metrics', () => {
    ...
    it('should handle high-frequency API calls', async () => {
      ...
      const promises = [];
      for (let i = 0; i < 10; i++) {
        promises.push(api.get(`/api/test-${i}`));
      }
